...
- [`server/templates/tools/`](https://github.com/datacommonsorg/website/tree/master/static/custom_dc/custom){: target="_blank"}: JSON files for visualization tools
- [`server/config/custom_dc/custom/`](https://github.com/datacommonsorg/website/blob/master/server/config/custom_dc/custom/base/header.json){: target="_blank"}: JSON files for layout elements
...
1. Create a new subdirectory under `static/custom_dc/` using the same name you created in step 1.
   ...
   mkdir yourproject
   ```
1. Copy [`static/custom_dc/custom/overrides.css`](https://github.com/datacommonsorg/website/blob/master/server/static/custom_dc/custom/overrides.css){: target="_blank"} into this directory. You can use this file to define your own styles.
   ```
   cp custom/overrides.css yourproject/
   ```
...
1. In your copied `base.html` file, remove the header `main-header` ID (or rename to something else), and add HTML elements in the tags. 
   ...
   ```
1. If you have your your own JS file(s), add them to your <code>static/custom_dc/<var>PROJECT_NAME</var></code> directory and add script elements to the `head` section of `base.html`. For example:
   ```{% raw %}
   <head>
   ...
   <script src={{url_for('static', filename='my_file.js', t=config['VERSION'])}} async></script>
