...
| query <br /> <required-tag>Required</required-tag> | string | A SPARQL query string.<br/>In the query, all desired entities must be specified; wildcards are not supported. Each node or entity must have a `typeOf` condition, for example, <code>?<var>ENTITY_NAME</var> typeOf City</code>.                                              |
{: .doc-table }
