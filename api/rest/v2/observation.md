...
| facets | object | Various properties of reported facets, where available. The following are always returned: <br/>`importName`: The name of the [provenance](/data_model.html#sources) or [dataset](/data_model.html#sources).<br/>`provenanceUrl`: The URL of the provenance or dataset.<br/> All others are only returned if they are set in the source dataset. `is_dc_aggregate` is only set to `true` for variables that are auto-generated by Data Commons to aggregate data by place hierarchies. |
{: .doc-table}
